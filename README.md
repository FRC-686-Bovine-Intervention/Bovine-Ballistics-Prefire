...
- [ ] Make game-agnostic (more of an offseason task)
##
### Using the simulator:
...
- [x] Windows support
- [x] Linux support
- [ ] MacOS support (planned but will be untested)
...
- **--timescale x** affects the speed at which the program runs. Data is not lost at faster speeds; just this argument isn't useful when visualizing. Does not affect the CLI version, so it is practically useless.
...
- **minVFly**: the minimum surface speed of the flywheel, in meters/second (recommended to keep as 0)
- **vFlyMaxTries**: the number of tries the simulator will take with various speeds before giving up on a combination of x, vx, and angle
####
...
####
- **robustnessFactor**: Arbitrary scalar defining the weight on the cost of the robustness of a trajectory
